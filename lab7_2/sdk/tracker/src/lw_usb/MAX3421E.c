...
void MAXreg_wr(BYTE reg, BYTE val) {
	int ret = -1;
	size_t write_buffer_size = 2;
	BYTE write_buffer[write_buffer_size];

	//select MAX3421E 
	XSpi_SetSlaveSelect(&SpiInstance, 1);

	//write reg + 2 via SPI (+2 for setting direction to write)
	...
	ret = XSpi_Transfer(&SpiInstance, write_buffer, NULL, write_buffer_size);

	//if return code != 0 print an error
	if (ret != XST_SUCCESS)
	{
		xil_printf("ERROR: MAXreg_wr failed with error code %x\n", ret);
	}
...
BYTE* MAXbytes_wr(BYTE reg, BYTE nbytes, BYTE* data) {
	int ret = -1;
	size_t write_buffer_size = nbytes+1;
	BYTE* write_buffer = malloc(write_buffer_size);
	...
	if (ret != XST_SUCCESS)
	{
		xil_printf("ERROR: MAXbytes_wr failed with error code %x\n", ret);
	}
...
BYTE MAXreg_rd(BYTE reg) {
	...
	//read val via SPI
	write_buffer[1] = dummy_bytes;

	//read return code from SPI peripheral 
	ret = XSpi_Transfer(&SpiInstance, write_buffer, read_buffer, buffer_size);

	//if return code != 0 print an error
	if (ret != XST_SUCCESS)
	{
		xil_printf("ERROR: MAXreg_rd failed with error code %x\n", ret);
	}
...
BYTE* MAXbytes_rd(BYTE reg, BYTE nbytes, BYTE* data) {
	int ret = -1;
	size_t buffer_size = nbytes + 1;
	BYTE* write_buffer = calloc(buffer_size, sizeof(reg));
	BYTE* read_buffer = calloc(buffer_size, sizeof(reg));
	...
	//read data[n] from SPI, where n goes from 0 to nbytes-1 (dummy bytes of 0 set in calloc)
	...
	if (ret != XST_SUCCESS)
	{
		xil_printf("ERROR: MAXbytes_rd failed with error code %x\n", ret);
	}
	else
	{
		memcpy(data, read_buffer+1, nbytes);
	}
